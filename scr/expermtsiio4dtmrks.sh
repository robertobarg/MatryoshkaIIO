...
DDIR=$(mktemp -d -p $(pwd) iio4dtmrk.of1.XXXXXXXX) && \
    sed -e "s/_ALG_/ts/" -e "s/_OF_/1/" dirfile.csv > matrydirfile.csv && \
    ./scr/multidir.sh scr/multicfg.sh scr/batchpl.sh $EXE matrydirfile.csv $DDIR $PRS

# run matryoshka experiments: OF 2
DDIR=$(mktemp -d -p $(pwd) iio4dtmrk.of2.XXXXXXXX) && \
    sed -e "s/_ALG_/ts/" -e "s/_OF_/2/" dirfile.csv > matrydirfile.csv && \
    ./scr/multidir.sh scr/multicfg.sh scr/batchpl.sh $EXE matrydirfile.csv $DDIR $PRS

# run matryoshka experiments: OF 3
DDIR=$(mktemp -d -p $(pwd) iio4dtmrk.of3.XXXXXXXX) && \
    sed -e "s/_ALG_/ts/" -e "s/_OF_/3/" dirfile.csv > matrydirfile.csv && \
    ./scr/multidir.sh scr/multicfg.sh scr/batchpl.sh $EXE matrydirfile.csv $DDIR $PRS

# run matryoshka experiments: OF 4
DDIR=$(mktemp -d -p $(pwd) iio4dtmrk.of4.XXXXXXXX) && \
    sed -e "s/_ALG_/ts/" -e "s/_OF_/4/" dirfile.csv > matrydirfile.csv && \
    ./scr/multidir.sh scr/multicfg.sh scr/batchpl.sh $EXE matrydirfile.csv $DDIR $PRS
